...
A=[]
B=[]
MinCut=1000000000000000000000000000000000000000
b=0
...
def MinimumCutPhase(G,w,a):
    ...
    for v,weights in enumerate(G.G[A[-1],:]):
        if weights != 0 and v != A[-2]:
            G.G[A[-2],v] = G.G[A[-2],v]+weights
            G.G[v,A[-2]] = G.G[v,A[-2]]+weights
        G.G[A[-1],v] = 0
        G.G[v,A[-1]] = 0
    G.Vertex.remove(A[-1])
    return CutOfPhase,A,B
...
if __name__ == "__main__":
    ...
    print("The Final Min Cut of the Graph is :",MinimumCut(G,w,a))
    elapsed=t()-start
...
###################################################################################################
############################## FILES ##############################################################
############ The following file show us the system when we run our program ########################
    ...
    f.write(f"The Final Min Cut of  the Graph is : {MinimumCut(G,w,a)}\n")
    f.close()
